...
Try {
    ...
    [String]$appName = 'Azure Data Studio'
    [String]$appVersion = ''
    [String]$appArch = ''
    [String]$appLang = 'DE'
    ...
    If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$AnyFilePath = Get-ChildItem -Path "$dirFiles" -Include azuredatastudio-windows-setup*.exe -File -Recurse -ErrorAction SilentlyContinue

        If($AnyFilePath.Exists) {
			Execute-Process -Path "$AnyFilePath" -Parameters "/VERYSILENT /NORESTART /MERGETASKS=!runcode /LOG=$configToolkitLogDir\AzureDataStudio_Install.log" -WindowStyle Hidden
        }
    ...
    ElseIf ($deploymentType -ieq 'Uninstall') {
		...
		$SourceCache = Get-InstalledApplication -Name 'Azure Data Studio'
        
        ForEach ($item in $SourceCache) {
			If($item.UninstallString) {
				$Uninstall = $item.UninstallString -replace '"', ''
			
				If(Test-Path -Path $Uninstall) {
					Execute-Process -Path $Uninstall -Parameters "/VERYSILENT /NORESTART /LOG=$configToolkitLogDir\AzureDataStudio_Uninstall.log"
